"use strict";
const TexturedIndexedTrianglesGeometry = function(gl, json) {
  this.gl = gl;
  ...
  gl.bufferData(gl.ARRAY_BUFFER,
    new Float32Array(json.vertices),//第三列shows the z value, the first and second row shows x and y value of triangle
    gl.STATIC_DRAW);

  this.vertexNormalBuffer = gl.createBuffer(); 
  gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexNormalBuffer); 
  gl.bufferData(gl.ARRAY_BUFFER, 
    new Float32Array(json.normals), 
    gl.STATIC_DRAW); 

  this.vertexTexCoordBuffer = gl.createBuffer(); 
  gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexTexCoordBuffer); 
  gl.bufferData(gl.ARRAY_BUFFER, 
    new Float32Array(json.texturecoords[0]), 
    gl.STATIC_DRAW);

var indices = [].concat.apply([], json.faces);
this.indicesLength = indices.length;
  ...
  //The second binding override the first binding
   gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexNormalBuffer);
