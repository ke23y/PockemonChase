...
{ 
  this.position = new Vec3(5.0, 5.0, 10.0); 
...
PerspectiveCamera.prototype.move = function(dt, keysPressed,t) { 
  if(this.isDragging){ 
  ...
  //Tracking 
  if (keysPressed.T) {
    this.position.x = 16* Math.pow(Math.sin(t),3) // x value of point
    this.position.y = 13*Math.cos(t) - 5*Math.cos(2*t) - 2*Math.cos(3*t) -Math.cos(4*t); // y value of point
    this.updateViewMatrix(); 
  }
